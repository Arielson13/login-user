...
export const TodoList: React.FC = () => {
  ...
  <ul className="list-group mt-3">
    {tasks.map((task) => (
      <li key={task.id} className="list-group-item d-flex justify-content-between align-items-center mb-3">
        <div>
          <input
            type="checkbox"
            className="me-2"
          ...
          <p className={`mb-0 text-muted ${task.completed ? "text-decoration-line-through" : ""}`}>
            {task.description}
          </p>
          <small className={`text-muted ${task.completed ? "text-decoration-line-through" : ""}`}>
